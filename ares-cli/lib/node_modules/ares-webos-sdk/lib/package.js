...
(function () {
    ...
    function Packager(options) {
        ...
        if (options && options.hasOwnProperty('override-appinfo')) {
            this.appInfoOverrides = JSON.parse(options['override-appinfo']);
        }
        ...
        checkInputDirectories: function(inDirs, options, next) {
            ...
            async.forEachSeries(inDirs, checkDirectory.bind(this, options),
                function(err, results) {
                    ...
                    // FIXME: additional validation for sinage
                    commonTools.appdata.compareProfile('signage', function(err, isSignage) {
                            if (isSignage &&
                                self.appDir &&
                                fs.existsSync(path.join(self.appDir, 'content'))) {
                                return setImmediate(next, "ERROR: APP_DIR should not contain 'content' folder or file");
                            }
                            setImmediate(next);
                    });
                }.bind(this));
    ...
    function overrideAppInfo(next) {
        log.verbose("overrideAppInfo");
        var o = this.appInfoOverrides;
        if (o) {
            for (var prop in o) {
                this.appinfo[prop] = o[prop];
            }
        }
        setImmediate(next);
    }
    ...
    function checkAppInfo(next) {
        ...
        if (this.appinfo.id.length < 1 || !(/^[a-z0-9.+-]*$/.test(this.appinfo.id))) {
            ...
            return setImmediate(next, errMsgHndl.changeErrMsg("INVALID_APPID"));
        }
        var schemaFile = path.join(__dirname, "schema/ApplicationDescription.schema");
    ...
    function changeCwd(dir, next, args) {
        //FIXME: this function exists due to the enyo-dev packaging limitation.
        //       It works fine only when it runs on app directory
        this.cwd = process.cwd();
        process.chdir(dir);
        if (next && typeof next == 'function') next(args);
    }

    function recoverCwd(next, args) {
        if (this.cwd) process.chdir(this.cwd);
        if (next && typeof next == 'function') next(args);
    }

    function buildEnyo(next) {
        if(fs.existsSync(path.join(this.appDir, 'lib/enyo'))){
            try {
                var enyoModule = require(path.join(this.appDir, 'lib/enyo')),
                    logLevelMapper = {
                        //npmlog: silly, verbose, info, http, warn, error
                        //enyo-dev: trace, debug, info, warn, error, fatal
                        'silly': 'trace',
                        'verbose': 'debug',
                        'info': 'info',
                        'http': 'info',
                ...
                if(enyoModule.version && enyoModule.version >= '2.6.0') {
                    console.log("Building the application using enyo-dev tool");
                    var enyoPack = require('enyo-dev').packager,
                        self = this,
                        enyoConfig = path.join(self.appDir, '.enyoconfig'),
                        newAppDir = path.join(self.appDir, 'dist'),
                        opts = {
                            package : this.appDir,
                            devMode : !this.minify,
                            sourceMaps : false,
                            logLevel: logLevelMapper[log.level] || 'warn',
                            outFile : this.appinfo.main
                        };

                    changeCwd(this.appDir);
                    if (fs.existsSync(enyoConfig)){
                        try{
                            var enyoConfigData = JSON.parse(fs.readFileSync(enyoConfig));
                            newAppDir = path.join(self.appDir, enyoConfigData.outdir || 'dist');
                            opts["lessPlugins"] = enyoConfigData.lessPlugins || [];
                        } catch(e){
                            log.warn("buildEnyo()# parsing error " + enyoConfig);
                            log.warn("buildEnyo()# err: " +  e);
                            log.warn("buildEnyo()# ignoring " + enyoConfig);
                        }
                    }
                    if (fs.existsSync(newAppDir)) {
                        shelljs.rm('-rf', newAppDir);
                    }
                    enyoPack(opts)
                        .on('end', function(){
                            defaultAssetsFields['main'] = false;
                            shelljs.cp('-rf', path.join(self.appDir, 'appinfo.json'), newAppDir);
                            self.appDir = newAppDir;
                            console.log("Success: the deployable application is available in:", newAppDir);
    ...
    function runDeployOldEnyo(next) {
        log.verbose("runDeployOldEnyo");
        if (this.appCount === 0) {
            return setImmediate(next);
        }
        var deployJs;
        if (this.minify === true) {
            ...
            if (fs.existsSync(path.join(this.appDir, "package.js"))) {
                if (this.appinfo.main && this.appinfo.main.match(/(\.html|\.htm)$/gi))
                {
                    ...
                    if (data.toString().match(regex)) {
                        ...
                        if (!deployJs) deployJs = path.join(this.appDir, "enyo/tools/deploy.js");
                    }
                }
            }
            ...
            if (fs.existsSync(deployJs)) {
                ...
                if (this['deploy-lib']) {
                    //FIXME: (1/2)
                    // (*) TODO: need to verify deploy.js, minify.js in enyo regarding to mapfrom/to
                    // If -l is specified with relative path, mapFrom also should be specified with the relative path
                    // So, just use "lib" path for -l option
                    //relLibPath = path.relative(path.resolve(this.appDir), path.resolve(this['deploy-lib']));
                    //deployArgs.push("-l", relLibPath);
                    deployArgs.push("-l", "lib");
                }
                ...
                if (this.appinfo.resolutionIndependent) {
                    log.verbose("Enabling resolution-independence...");
                    deployArgs.push("-ri");
                }
                            ...
                            if (fs.existsSync(src)) {
                                if (minify && '.js' === path.extname(src) && file.relPath.indexOf('node_modules') === -1 ) {
                                    ...
                                    try {
                                        ...
                                        fs.writeFileSync(path.join(dstPath, file.relPath), data);
                                    } catch (e) {
    ...
    function getFileSize(subdir, next) {
        log.verbose("getFileSize");
        var self = this;
        var inPath = path.join(this.tempDir, subdir);
        async.waterfall([
            _readSizeRecursive.bind(this, inPath)
        ], function(err, size){
            if (!err && size) {
    ...
    function makeIpk(destination, next) {
        var self = this;
        this.ipk = path.join(destination, this.ipkFileName);
        log.verbose("makeIpk in dir " + destination + " file " + this.ipkFileName );
		...
		ipkStream.on('close', function() {
			console.log("Creating package " + self.ipkFileName + " in " + destination);
			setImmediate(next);
		});
