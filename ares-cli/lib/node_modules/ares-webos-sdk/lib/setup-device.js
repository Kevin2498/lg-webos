...
(function () {
    ...
    function isValidPort(port) {
        return new String(port).match(/^[0-9]+$/)
    }
            ...
            function(devices, next){
                if(Array.isArray(devices)){
                    devices.forEach(function(device){
                        var info, item, conn = device.conn.concat([]);
                        ...
                        var data = (mode === 'full') ? item : {name: device.name, info:info, connection: (device.conn || 'ssh'), profile: device.profile };
                        datas.push(data);
                    });
                }
                next(datas);
    ...
    devicetools.showDeviceListAndExit = function(mode, next){
        var mode = mode || 'list';
        async.waterfall([
