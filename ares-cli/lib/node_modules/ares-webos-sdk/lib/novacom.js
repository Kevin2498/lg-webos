...
(function() {
    ...
    novacom.Resolver.prototype = {
        ...
        load: function(next) {
            ...
            function _loadString(next) {
                ...
                var inDevices = cliData.getDeviceList(true);
            ...
            function _loadUSBDevice(next) {
                log.silly("Resolver#load#_loadUSBDevice()");

                async.waterfall([
                    novacomusb.getDeviceList.bind(novacomusb, {
                        appdir: appdir
                    }),
                    function(usbDeviceList, next) {
                        async.forEach(usbDeviceList, function(usbDevice, next) {
                            async.series([
                                resolver._loadOne.bind(resolver, usbDevice),
                                resolver._addOne.bind(resolver, usbDevice)
                            ], next);
                        }, next);
                    }
                ], function(err, results) {
        ...
        _addOne: function(inDevice, next) {
            ...
            var systemTypes = cliData.getCommandService();
        ...
        setDefaultName: function(target, next) {
            var name = (typeof target === 'string' ? target : target && target.name);
            if (!name) {
                var usbDevices = this.devices.filter(function(device) {
                    return (device.conn && device.conn.indexOf("novacom") !== -1);
                });
                if (usbDevices.length > 1) {
                    return next(new Error("multiple devices are connected by novacom, please specify target name."));
                }
                // default target priority
                // specified target name > target connected by novacom > emulator
        ...
        modifyDeviceFile: function(op, target, next) {
            ...
            if (!target.name) {
                return setImmediate(next, new Error("Incorrect target name"));
            }
            if (!Array.isArray(inDevices)) {
                return setImmediate(next, new Error("Incorrect file format'"));
            }
            matchedDevices = inDevices.filter( function(dev) {
                var match = false;
                if (target.name === dev.name) {
                    if (target.profile) {
                        if (target.profile === dev.profile) { match = true; }
                    } else {
                        match = true;
                    }
                }
            ...
            if (op == 'add') {
                if (matchedDevices.length > 0) {
                    return setImmediate(next, new Error("Existing Target Name"));
                }
                for (key in target) {
            ...
            } else if (op === 'remove' || op === 'modify') {
                var matchNames,
                    errMsg;
                if (matchedDevices.length === 0) {
                    return setImmediate(next, new Error("There is no device named " + target.name));
                }
                matchNames = matchedDevices.map(function(dev) {
                    return dev.name;
                });
                if (op === 'remove') {
                    inDevices = inDevices.filter(function(dev) {
                        if (matchNames.indexOf(dev.name) !== -1) {
                            if (dev.indelible === true) {
                                errMsg = dev.name + " is not removable";
                                return true;
                            } else {
                                return false;
                            }
                ...
                return setImmediate(next, new Error("Unknown operator #" + op));
            }
    ...
    function Session(target, next) {
        ...
        async.waterfall([
            ...
            this.resolver.setDefaultName.bind(this.resolver, target),
    ...
    novacom.Session.prototype = {
            ...
            self.sftpPut(inPath, outPath, function(err) {
                if (err) {
                    ...
                    if (4 === err.code || 127 === err.code) {
                        ...
                        var detailMsg = errMsgHndl.changeErrMsg("insufficient free space");
                ...
                transfer: function(next) {
                    self.ssh.sftp(function(err, sftp) {
                        ...
                        readStream.on('error', function(err) {
                            log.verbose('Session#sftpGet()', "error:", err);
                            setImmediate(next, err);
                        });
        ...
        run: function(cmd, stdin, stdout, stderr, next) {
            if (this.ssh) {
                ...
                novacomusb.runDeviceCommand(this.target.id, cmd, stdin, stdout, stderr, next);
            }
        },
         ...
         * @param {Function} next commonJS callback invoked upon completion or failure
         */
        run_ssh: function(cmd, stdin, stdout, stderr, next) {
            var err;
        ...
        runNoHangup: function(cmd, cbData, cbExit, next) {
            if (this.ssh) {
                ...
                novacomusb.runDeviceCommand(this.target.id, cmd, null, null, null, next);
            }
        },
