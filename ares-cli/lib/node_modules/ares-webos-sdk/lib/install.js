...
(function() {
        ...
        install: function(options, hostPkgPath, next) {
            ...
            async.waterfall([
                ...
                function(storageList, next) {
                    ...
                    if (storage && config.changeTempDir) {
                        config.tempDirForIpk = path.join(storage.uri, 'temp').replace(/\\/g,'/');
                    ...
                    function _appinstalld(next) {
                        ...
                        luna.send(options, addr, param, function(lineObj, next) {
                            ...
                            if (Array.isArray(resultValue)) {
                                // success: stop
                                if (!cliData.compareProfileSync('signage')) {
    ...
    function _getStorageList(options, next) {
        ...
        luna.send(options, addr, param, function(lineObj, next) {
            ...
            if ( err && err.toString().indexOf("not exist") !== -1 ) {
                return next(null, storageList);
            }
            next(err, value);
        });
    }
