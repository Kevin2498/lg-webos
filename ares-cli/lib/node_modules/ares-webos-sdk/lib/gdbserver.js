...
var prefixAppPath = prefixPath + "apps/usr/palm/applications";
var prefixServicePath = prefixPath + "apps/usr/palm/services";
...
(function() {
    ...
    var gdbserver = {
        ...
        run: function(options, params, next) {
            ...
            function _checkGdbserver(session, next) {
                var cmd = "if ! type gdbserver > /dev/null; then echo 1; else echo 0; fi;";
                this.session = session;
                session.run(cmd, null, _onData, null,
                        function(err) {
                            if (err) return setImmediate(next, err);
                        }
                );
                function _onData(data) {
                    var notFound = (Buffer.isBuffer(data))? data.toString().trim() : data.trim();
                    if (notFound == "1") {
                        return next(new Error("gdbserver command is not available in the target device"));
                    } else {
                        setImmediate(next);
                    }
                }
            ...
            function _readAppInfo(next) {
                ...
                var id = appId || svcId;
                var prefix = prefixAppPath;

                if (options.instPkgs) {
                    var suffix = (appId)? "applications" : "services";
                    options.instPkgs.every(function(pkg) {
                        if (id.indexOf(pkg.id) !== -1) {
                            prefixAppPath = prefixServicePath = path.join(path.dirname(pkg.folderPath), '..', suffix).replace(/\\/g, '/');
                            return false;
                        }
                        return true;
                    });
                ...
                if (appId) {
                    var metaFilePath = path.join(prefixAppPath, appId, 'appinfo.json');
                } else if (svcId) {
                    var metaFilePath = path.join(prefixServicePath, svcId, 'services.json');
                } else {
                ...
                async.series([
                    ...
                    if (err) {
                        return next(new Error(id + " was not installed"));
                    }
                });
            ...
            function _getExecFileName(metaData, next) {
                ...
                try {
                    ...
                    if (appId) {
                        ...
                        if (metaInfo.type === "web") {
                            return next(new Error(metaInfo.id + " is not a native app"));
                        }
            ...
            function _addUserEnv(systemEnv, next) {
                ...
                if (appId) {
                    var envVariables = {
                        ...
                        "LD_LIBRARY_PATH" : "$LD_LIBRARY_PATH:" + path.join(prefixAppPath, appId, 'lib').replace(/\\/g, "/")
                    };
                } else if (svcId) {
                    var envVariables = {
                        //"USER_VARS" : "VALUE",
                        "LD_LIBRARY_PATH" : "$LD_LIBRARY_PATH:" + path.join(prefixServicePath, svcId, 'lib').replace(/\\/g, "/")
                    };
                }
            ...
            function _launchGdbserver(prefixEnv, next) {
                ...
                var format = "cd %s && gdbserver :%s %s";
                var dirPath,
                    cmdRunGdbserver;
                if (appId) {
                    dirPath = path.join(prefixAppPath, appId);
                } else if (svcId) {
                    dirPath = path.join(prefixServicePath, svcId);
                }
                cmdRunGdbserver = util.format(format, dirPath, this.port, path.join(dirPath, this.execName))
                                    .replace(/\\/g, "/");
            ...
            function _portForward(next) {
                ...
                options.session.forward(this.port, this.port, next);
            }

            function _printForwardInfo(next) {
                log.verbose("gdbserver#run()#_waitExit");
                var target = options.session.getDevice();
                console.log(" >> gdb can connect to [target remote", target.host+":"+this.port+"]\n");
                next();
            }
        },
