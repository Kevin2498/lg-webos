...
(function() {
    ...
    var push = {
        ...
        push: function(options,next) {
            ...
            var novacom_path = novacomUsb.getNovacomPath();
            var session;
            commonTools.appdata.compareProfile('watch', function(err, watch) {
                isWatch = watch;
            });
                            ...
                            session.run(cmd, null, writeIsDir, null, function() {
                                ...
                                if (writeIsDir.getContentsAsString() == 'd\n') {
                                    ...
                                    if(!isWatch) {   
                                        ...
                                        if (deviceId) {
                                            ls = spawn(novacom_path, ["run", "file:///bin/mkdir " + basedir, "-d", deviceId]);
                                        }    
                                        else {
                                            ls = spawn(novacom_path, ["run", "file:///bin/mkdir " + basedir]);
                                        }
                                        ls.stdin.end();
                            ...
                            async.eachSeries(arrFiles, function(item, callback) {
                                ...
                                if (!stat.isDirectory()) {
                                    ...
                                    if(!isWatch) {
                                        ...
                                        if (deviceId) {
                                            ls = spawn(novacom_path, ['put', 'file://' + filepath, '-d', deviceId]);
                                        }
                                        else {
                                            ls = spawn(novacom_path, ['put', 'file://' + filepath]);
                                        }  
                                        totalSize += stat.size;
                                        iCount++;
                                        var file = fs.createReadStream(dfPath);
                                ...
                                //Create sub-directories
                                    ...
                                    function _createsubDir(next) {
                                        ...
                                        if(!isWatch) {
                                            ...
                                            if (deviceId) {
                                                subdir = spawn(novacom_path, ["run", "file:///bin/mkdir " + destPath, "-d", deviceId]);
                                            }    
                                            else {
                                                subdir = spawn(novacom_path, ["run", "file:///bin/mkdir " + destPath]);
                                            }
                                            subdir.stdin.end();
                                            subdir.stderr.on('data', function(data) {
                                                setImmediate(next, data);
                                            });
                                            subdir.on('close', function(code) {
                                                setImmediate(next,null,dfPath,true);
                                            });
                                        }
                                    }
                          ...
                          function transferFile(next) {
                              ...
                              session.run(cmd, null, IsDir, null, function() {
                                  ...
                                  if(!isWatch) {
                                      ...
                                      totalSize += stats.size;
                                      iCount++;
                                      var file = fs.createReadStream(sourcePath);
                                      file.on('error', function(err) {
                                          setImmediate(next, err);
                                      });
                                      file.on('data', function(data) {
