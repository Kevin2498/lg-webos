                        ...
                        if(writeIsFile.getContentsAsString() == 'f\n') {
                            ...
                            fs.exists(destinationPath, function(exists) {
                                ...
                                if(!isWatch) {
                                    ...
                                    if(deviceId) {
                                        ls = spawn(novacom_path, ['get', 'file://'+ sourcePath,'-d',deviceId]);
                                    }
                                    else {
                                        ls = spawn(novacom_path, ['get', 'file://'+ sourcePath]);
                                    }
                                    iCount++;
                                    var output_file=fs.createWriteStream(destinationPath, {encoding: 'binary'});
                                    ...
                                    function _copyAllFiles(next) {
                                        ...
                                        session.run(cmd, null, writeFileList, null, function() {
                                            ...
                                            if(!isWatch) {
                                                ...
                                                var ls;
                                                if(deviceId) {
                                                    ls = spawn(novacom_path, ['get', 'file://'+ item.replaceAll(" ","\\ "),'-d',deviceId]);
                                                }
                                                else {
                                                    ls = spawn(novacom_path, ['get', 'file://'+ item.replaceAll(" ","\\ ")]);
                                                }
                                                iCount++;
                                                var filepath=path.join(destinationPath, item.substring(dirIndex));
                                                var output_file=fs.createWriteStream(filepath, {encoding: 'binary'});
                                                ls.stdout.on('data', function (data) {
                                                    output_file.write(data);
                                                });
                                                ls.stdin.end();
